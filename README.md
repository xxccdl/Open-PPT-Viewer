...
- **打开**：`.pptx` / `.pptm` / `.ppsx` / `.ppsm` / `.potx` / `.potm` 与 `.pdf`，
  拖进去或双击都能开；旧版 `.ppt` 由本机办公软件自动转成 `.pptx` 后打开
  （转一次约 0.9s，结果缓存在 `cache/legacy/`，第二次直接命中）
...
- **只支持 Windows**：按步出图、格式转换、安装器、文件关联都依赖 Windows 的东西
- **强调动画是近似**：变色、放大这类动画不改变可见性，而静态帧表达不了它，
  所以这一步仍由自研内核合成
- **旧版 `.ppt` 需要本机装有 WPS 或 PowerPoint**：转换就是交给它做的；
  没装的话会退回「交给系统默认程序打开」并说明原因
- **磁盘缓存**：办公软件出的逐页图与动画分帧会占空间
  （实测某份 39 页含动画的课件约 180MB）。分帧只出一次，
  但**「办公软件出的图」目前没有自动淘汰**（位图缓存的淘汰只管自己那棵子树）
